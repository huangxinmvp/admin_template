...
renderer.on('file-context:file-closed', function(filePath, done) {
  ...
  const processApplicationFile = fileContext._indexer
    .getItems()
    .find((item) => {
      return (
        path.dirname(filePath).startsWith(path.dirname(item.file.path)) &&
        isProcessApplicationFile(item.file.path)
      );
    });
...
app.createEditorWindow = function() {
  ...
  // 监听 F12 或 Ctrl+Shift+I 手动打开 DevTools
...
function bootstrap() {
  ...
  function onIndexerUpdated() {
    ...
    const added = items.filter(
      ({ uri }) => !lastItems.find(({ uri: lastUri }) => uri === lastUri)
    );
    const removed = lastItems.filter(
      ({ uri }) => !items.find(({ uri: newUri }) => uri === newUri)
    );
    ...
    renderer.send(
      ...
      items.map(({ file, metadata }) => ({ file, metadata }))
    );
  }
